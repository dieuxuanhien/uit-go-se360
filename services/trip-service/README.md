...
#### POST /trips/{trip_id}/arrive-pickup

Mark that a driver has arrived at the pickup location for an en route trip.
...
- **400 Bad Request** - Trip status is not EN_ROUTE_TO_PICKUP
  ```json
  {
    "statusCode": 400,
  ...
  ```
- **401 Unauthorized** - Missing or invalid JWT token
- **403 Forbidden** - Different driver attempting to mark arrival
  ```json
  {
    "statusCode": 403,
...
```bash
curl -X POST "http://localhost:3002/trips/123e4567-e89b-12d3-a456-426614174000/arrive-pickup" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```

#### POST /trips/{trip_id}/start-trip

Mark that a driver has picked up the passenger and the trip is now active.
...
- **400 Bad Request** - Trip status is not ARRIVED_AT_PICKUP
  ```json
  {
    "statusCode": 400,
...
```bash
curl -X POST "http://localhost:3002/trips/123e4567-e89b-12d3-a456-426614174000/start-trip" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```
