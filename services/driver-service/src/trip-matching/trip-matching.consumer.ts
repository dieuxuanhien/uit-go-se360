...
/**
 * Story 2.1: SQS Consumer for Trip Matching
 * Polls driver-match-queue for TripRequested events
 */
@Injectable()
export class TripMatchingConsumer implements OnModuleInit {
  private readonly logger = new Logger(TripMatchingConsumer.name);
  private queueUrl!: string;
  private isRunning = false;
  private readonly processedMessageIds = new Set<string>();

  constructor(private readonly tripMatchingService: TripMatchingService) {}

  /**
   * Start polling when module initializes
   */
  async onModuleInit() {
    this.logger.log('Initializing Trip Matching Consumer...');
    this.queueUrl = await getQueueUrl('driver-match-queue');
  ...
  private async startPolling() {
    ...
    this.isRunning = true;
    this.logger.log('🚀 Starting SQS polling for trip-matching queue...');
  ...
  private async processMessage(message: any) {
    ...
    try {
      ...
      this.logger.log('📨 Processing TripRequested event', {
        messageId,
        tripId: event.tripId,
      ...
      // Validate event type
      if (event.eventType !== 'TripRequested') {
      ...
      // Process trip matching
      await this.tripMatchingService.matchDriverForTrip({
        tripId: event.tripId,
