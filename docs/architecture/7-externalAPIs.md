...
exportclassFareCalculator {

privatereadonlyBASE_FARE = 200; // $2.00 in cents

privatereadonlyPER_KM_RATE = 150; // $1.50 per km in cents

privatereadonlyPER_MINUTE_RATE = 30; // $0.30 per minute in cents


calculateEstimatedFare(distanceKm: number): number {

constdistanceFare = Math.round(distanceKm * this.PER_KM_RATE);


// Estimate duration: assume 30 km/h average speed

constestimatedMinutes = (distanceKm / 30) * 60;

consttimeFare = Math.round(estimatedMinutes * this.PER_MINUTE_RATE);


returnthis.BASE_FARE + distanceFare + timeFare;
...
constR = 6371; // Earth radius in km

constdLat = this.toRad(lat2 - lat1);

constdLng = this.toRad(lng2 - lng1);
...
privatetoRad(degrees: number): number {
...
exportclassMapsClient {

asyncreverseGeocode(lat: number, lng: number): Promise<string> {
...
asynccalculateRoute(origin, destination): Promise<RouteInfo> {
...
returnawaitthis.mapsClient.calculateRoute(origin, destination);

} else {

returnthis.fareCalculator.calculateEstimatedFare(distance);
...
constcacheKey = `route:${originLat},${originLng}:${destLat},${destLng}`;

constcached = awaitthis.redis.get(cacheKey);

if (cached) returnJSON.parse(cached);


constresult = awaitthis.mapsClient.calculateRoute(origin, destination);

awaitthis.redis.setex(cacheKey, 3600, JSON.stringify(result)); // 1 hour TTL
